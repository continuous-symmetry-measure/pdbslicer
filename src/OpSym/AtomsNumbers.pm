...
sub new {
	...
	if  ( defined $numbers_string and  $numbers_string!~m/^([\d,]|-)+$/ ){
	    warn "The AtomsNumbers string '$numbers_string' contain non valid  characters.".
...
sub parse_numbers_string{
	...
	for my $n (@tmp){
		if ($n=~/^\d+$/){
			push @atoms_numbers,$n;
			
		}
		elsif ($n=~/^(\d+)-(\d+)/){
			my ($first,$last)=($1,$2);
			unless ($first<=$last){
				warn "The AtomsNumbers string '$numbers_string' contain non valid element:$n.";
				$self->{STATUS}="NON_VALID_ELEMENT_FIRST_GT_LAST-$n";
				next;
			}
			push @atoms_numbers,($first..$last);
		}
		else{
			warn "The AtomsNumbers string '$numbers_string' contain non valid  element:$n.";
	...
	my @unique 		= sort {$a<=>$b} (List::MoreUtils::uniq( @atoms_numbers )); 
	$self->{ATOMS_NUMBERS}	= [@unique];
	return $self->{STATUS};
		
}	# ----------  end of subroutine parse_numbers_string  ----------

sub get_atoms_numbers{
	my $self=shift;
	return $self->{ATOMS_NUMBERS};
